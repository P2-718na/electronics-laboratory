...
\section{Introduzione}\label{sec:scopo}
  ...
  Per correnti piccole, l'andamento previsto dalla teoria è riassunto dall'Equazione di Shockley
  \begin{equation}
    I(V) = I_0 \left(
      e^{
        \frac {V_d} {\eta V_T}
...
\section{Valori numerici delle misure}\label{sec:valori-misure}
  ...
  \subsection{Dati relativi al germanio}\label{subsec:valori-germanio}
    \begin{table}[H]
      ...
      \begin{tabular}[t]{c|c|c||c|c|c}
        \toprule
        V ($mV$) & V_\text{fondoscala} ($mV$) & \sigma_V ($mV$) & I ($mA$) & I_\text{fondoscala} ($mA$) & \sigma_I ($\mu mA$)%
        \csvreader[
          head to column names,
